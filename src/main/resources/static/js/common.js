...
//  });
	...
	com = {
		 ...
		 * */
	   tableRender:function(elem,id,height,limits,url,cols,page,totalRow,done){
			if(page==undefined||""){
				page = true;
			}
		...
		*  where---额外参数
		* */
		,pageOpen:function(title,content,area,where){
		...
		* * */
		...
		//-----------筛选数据改变index的iframe页面地址(非本页面！调用该方法成功后，会跳转到新的地址),即改变的iframe页面需要携带参数,------------
		/*
		 例如编辑
		* 参数：
		* content---跳转页面的url地址
		* tableId-----table的id（不是css的id）
		* * */
		,checkOpenPage:function(content,tableId){
					if(tableId==undefined||""){
						tableId = "tableId";
					}
					var checkStatus = table.checkStatus(tableId); //test即为基础参数id对应的值
					...
					parent.id_ = checkStatus.data[0].id;//传给子页面的数据id！！！！
					$(window.parent.document).find("#iframe").attr('src', content);
				}
